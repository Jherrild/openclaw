...
## Requirements
1. **Mapping Awareness:** The `scribe_save` and `scribe_move` tools must be aware of the Supernote mapping file at `skills/supernote-sync/sync-mapping.json`.
...
3. **Auto-Update on Move:** When `scribe_move` moves an existing Supernote note, it must update the `localPath` in the mapping file.
