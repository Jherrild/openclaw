...
- **Settings File:** `settings.json` (stores `preferred_areas` list for TTS routing priority)
...
3. Multiple rooms occupied → first match from `preferred_areas` in `settings.json` (if configured); falls back to all occupied rooms if no match
4. Single room occupied → that room's speaker
5. No room-level presence but someone is home → Living Room speaker (default fallback)
...
Update the `preferred_areas` list in `settings.json`. When `follow-and-speak` detects multiple occupied rooms, it picks the first room from this list that is occupied, rather than announcing in all rooms.

**Parameters:**
- `areas`: Comma-separated list of area names in priority order.

**Usage:**
```bash
node /home/jherrild/.openclaw/workspace/skills/home-presence/presence.js set-preference "Office,Gym,Bedroom"
```

**Output:** JSON confirming the updated preference list.

**Default list:** `["Office", "Bedroom", "Kitchen"]` (set in `settings.json`).
...
The ha-bridge integrates with the centralized [interrupt-service](../interrupt-service/SKILL.md). When a `state_changed` event matches an entity in the **dynamic watchlist**, ha-bridge forwards it to the interrupt service via `POST /trigger`.

**How it works:**
1. The interrupt-service pushes the active entity watchlist to ha-bridge on port 7601 (`POST /watchlist`) whenever rules change.
2. ha-bridge updates its in-memory `WATCHLIST` set immediately.
3. When a matching HA event fires, ha-bridge POSTs to `POST 127.0.0.1:7600/trigger`.
4. The interrupt-service handles rule matching, batching, rate limiting, and dispatch.

**Fallback:** ha-bridge also polls `GET /rules/ha-entities` every 15 minutes as a safety net.

**Managing interrupts:** Use the interrupt-service CLI — NOT the old `register-interrupt.js` (deprecated):
```bash
# Add an HA interrupt
node skills/interrupt-service/interrupt-cli.js add \
  ...
  --label "April arrived" --channel telegram

# List active interrupts
node skills/interrupt-service/interrupt-cli.js list

# Check ha-bridge watchlist health
curl http://127.0.0.1:7601/health
```

See [interrupt-service SKILL.md](../interrupt-service/SKILL.md) for full CLI reference.
...
- Forwards watched entities to the interrupt-service via `POST /trigger` (watchlist synced via push + 15-min fallback poll).
- Exposes HTTP server on port 7601 (`POST /watchlist`, `GET /health`) for interrupt-service push notifications.
